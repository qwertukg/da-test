        ...
        with PdfPages(pdf_path) as pdf:
            for i in sel:
                ...
                fig = visualize_pipeline(
                    ...
                    clf=clf,
                    true_label=y_test[i],
                    title=f"От стимула к смыслу — цифра 9 (test idx {i})",
                )
                pdf.savefig(fig)
                plt.close(fig)
        print(f"Сохранено в PDF: {pdf_path} (страниц: {len(sel)})")

    # Пример: сохраняем суммарную карту для цифры '9' по 100 примерам
