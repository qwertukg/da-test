...
Для ускоренного расчёта энергий пар на GPU, описанного в разделе 5.12 статьи [DAML.pdf](DAML.pdf), дополнительно установите PyTorch (сборку `+cpu` или CUDA в зависимости от конфигурации) и при создании раскладки передайте `device="cuda"` в `Layout2D`.
...
* Фаза FAR минимизирует локальную энергию `sim × dist`, переставляя пары точек, если после свапа сумма уменьшается. Вычисления энергий могут выполняться на GPU: каждая нить берёт одну пару, коды кешируются в локальной памяти блока, что повторяет схему раздела 5.12 DAML и позволяет использовать параллельный пересчёт без изменения математического алгоритма.
