...
`Layout2D` проверяет наличие `numpy.bit_count` (функция появилась в NumPy 2.0). Если библиотека собрана без неё (ветка 1.x),
раскладка автоматически переключается на обход через `numpy.unpackbits` и встроенный `int.bit_count`, поэтому обновление до 2.0
не обязательно.
