# -*- coding: utf-8 -*-
"""Основные компоненты пайплайна DAMP-light."""
...
class PrimaryEncoder:
    ...
    def __init__(
        ...
        grid_g: int = 4,
        grid_levels: int = 4,
        grid_bits_per_cell: int = 3,
        # COARSE
        coarse_bits_per_cell: int = 4,
        # BRIGHT
        bright_levels: int = 8,
        # ORIENT (Sobel)
        orient_on: bool = True,
        orient_bins: int = 8,
        orient_grid: int = 4,
        orient_bits_per_cell: int = 2,
        orient_mag_thresh: float = 0.12,
        # итоговый лимит
        max_active_bits: int = 260,
    ) -> None:
    ...
    def encode(self, img: np.ndarray) -> Set[int]:
                ...
                for x in range(self.orient_grid):
                    ...
                    for iy in range(h):
                        for ix in range(w):
                            b = int(
                                np.clip(
                                    np.searchsorted(bins, tile_a[iy, ix], side="right") - 1,
                                    0,
                                    self.orient_bins - 1,
                                )
                            )
        ...
        for y in range(G):
            for x in range(G):
                ...
                lvls = {lvl}
                if lvl > 0:
                    lvls.add(lvl - 1)  # лёгкое перекрытие
                for lv in lvls:
        ...
        out: Set[int] = set()
        for _, bit in ph:
            if len(out) >= self.max_active_bits:
                break
            out.add(bit)
        return out
...
class Layout2D:
    ...
    def __init__(self, R_far: int = 7, R_near: int = 3, epochs_far: int = 8, epochs_near: int = 6, seed: int = 123) -> None:
        self.R_far = R_far
        self.R_near = R_near
    ...
    def _local_energy(
        ...
        override: Optional[Dict[Tuple[int, int], int]] = None,
    ) -> float:
        """
        ...
        for ny, nx in self._neighbors(y, x, R):
            jdx = (
                override.get((ny, nx), self._cell_owner.get((ny, nx)))
                if override
                else self._cell_owner.get((ny, nx))
            )
            if jdx is None:
                continue
    ...
    def fit(self, codes: List[Set[int]]) -> "Layout2D":
        n = len(codes)
        ...
        def pass_epoch(R: int, iters: int) -> None:
            for _ in range(iters):
                ...
                for ia, yxa, ib, yxb in pairs:
                    e_cur = self._local_energy(yxa, ia, R, sim_cache) + self._local_energy(yxb, ib, R, sim_cache)
                    override = {yxa: ib, yxb: ia}
                    e_swp = self._local_energy(yxa, ib, R, sim_cache, override=override) + self._local_energy(
                        yxb, ia, R, sim_cache, override=override
                    )
                    if e_swp + 1e-9 < e_cur:
                        # применяем свап
...
class DetectorSpace:
    ...
    def __init__(
        ...
        emb_bits: int = 2048,
        lam_floor: float = 0.06,
        percentile: float = 0.88,
        min_activated: int = 35,
        mu: float = 0.20,
        seeds: int = 1200,
        min_comp: int = 5,
        min_center_dist: float = 1.6,
        max_detectors: int = 512,
        seed: int = 7,
    ) -> None:
        self.layout = layout
...
class KNNJaccard:
    def __init__(self, k: int = 5, eps: float = 1e-6) -> None:
        self.k = k
        self.eps = eps
        self.X: List[Set[int]] = []
        self.y: List[int] = []

    def fit(self, X: List[Set[int]], y: List[int]) -> "KNNJaccard":
        self.X = X
        self.y = y
        return self
